...
with open(f'{section}/info_sort.csv', 'r', encoding='utf-8', newline='') as file:
    ...
    for row in reader:
        ...
        with open(f'{section}/info_db.csv', 'a', encoding='utf-8', newline='') as file:
            writer = csv.writer(file, delimiter=';', dialect='excel')
            writer.writerow([id, 4, 6, f'{section}_{row["number"]}.jpg', f'{section}/preview_{section}/preview_{section}_{row["number"]}.jpg', f'{section}/main_{section}/{section}_{row["number"]}.jpg', track_id, row['name'].capitalize(), '2023-01-12', row['resolution'][:-3], imahe_height[0][:-1], imahe_height[1][1:], '["jpg"]', row['name'].capitalize(),  row['tags'].replace("'", '"'), '["ffffff"]', 0.00000000, 0, 1, 0, 1, 0, 0, 1, 1, 'NULL', 1, 0, '2023-01-12', '2023-01-13'])
        id += 1